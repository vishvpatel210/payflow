        ...
        <div className="w-full min-w-0">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
              ...
              <div className="flex flex-col md:flex-row flex-wrap items-center gap-3 w-full md:w-auto">
                ...
                <button 
                  ...
                  disabled={processing}
                  className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-70 text-white px-6 py-3 w-full md:w-auto rounded-xl text-sm font-bold shadow-lg shadow-indigo-600/20 transition-all flex items-center justify-center gap-2"
                >
          ...
          {loading ? (
            ...
            <>
              ...
              {/* ── Main Table ── */}
              <div className="bg-white md:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden mb-8">
                ...
                <div className="md:hidden divide-y divide-slate-50">
                  {filteredEmployees.map((emp, idx) => (
                    <div key={emp.employeeId || idx} className="p-5 flex flex-col gap-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                           <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${emp.name}`} className="w-10 h-10 rounded-full bg-slate-100 shrink-0" alt=""/>
                           <div>
                             <p className="text-sm font-bold text-slate-800">{emp.name}</p>
                             <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{emp.role}</p>
                           </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 items-end">
                         <div>
                           <p className="text-[10px] font-bold text-slate-400 uppercase">Net Salary</p>
                           <p className="font-bold text-slate-800 font-outfit">{formatCurrency(emp.netPay)}</p>
                         </div>
                         <div className="flex justify-end gap-2 items-center">
                            <button onClick={() => openEditModal(emp)} className="p-2 text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 rounded-lg transition-all"><Pencil size={16} /></button>
                            {emp.status !== 'Paid' ? (
                              <button onClick={() => handlePay(emp)} disabled={payingId === emp.employeeId} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50">
                                {payingId === emp.employeeId ? <Loader2 size={12} className="animate-spin" /> : 'Pay'}
                              </button>
                            ) : (
                              <button className="p-2 text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 rounded-lg transition-all"><ExternalLink size={16} /></button>
                            )}
                         </div>
                      </div>
                    </div>
