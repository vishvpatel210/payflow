...
const employeesData = [
  { id: 1, name: 'Elena Valerius', email: 'elena.v@atelier.com', role: 'Senior Brand Architect', department: 'DESIGN OPS', salary: '$142,500', status: 'ACTIVE', avatar: '1' },
  { id: 2, name: 'Julian Thorne', email: 'j.thorne@atelier.com', role: 'Lead Frontend Engineer', department: 'ENGINEERING', salary: '$168,000', status: 'ONBOARDING', avatar: '2' },
  { id: 3, name: 'Amara Okafor', email: 'a.okafor@atelier.com', role: 'Product Strategist', department: 'OPERATIONS', salary: '$135,200', status: 'ACTIVE', avatar: '3' },
  { id: 4, name: 'Marcus Hayes', email: 'm.hayes@atelier.com', role: 'Visual Designer', department: 'DESIGN OPS', salary: '$118,000', status: 'LEAVE', avatar: '4' },
];
    ...
    <Layout>
      ...
      <div className="flex gap-4 mb-6">
        ...
        <button className="px-4 py-3 bg-slate-50 border border-slate-200 hover:bg-white text-slate-600 rounded-xl font-semibold flex items-center gap-2 transition-colors text-sm">
          <Briefcase size={16} /> Department <ChevronDown size={14} className="ml-2 text-slate-400"/>
        </button>
        <button className="px-4 py-3 bg-slate-50 border border-slate-200 hover:bg-white text-slate-600 rounded-xl font-semibold flex items-center gap-2 transition-colors text-sm">
          <div className="w-4 h-4 rounded-full border-2 border-slate-300 flex items-center justify-center"><div className="w-1.5 h-1.5 rounded-full bg-slate-400"></div></div> Status <ChevronDown size={14} className="ml-2 text-slate-400"/>
        </button>
        <button className="px-4 py-3 bg-slate-50 border border-slate-200 hover:bg-white text-slate-600 rounded-xl font-semibold flex items-center gap-2 transition-colors text-sm">
          <Filter size={16} /> Sort
        </button>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden mb-8">
        ...
        <div className="divide-y divide-slate-50">
          {employeesData.map((emp) => (
            <div key={emp.id} className="flex items-center px-8 py-6 hover:bg-slate-50/50 transition-colors group">
              <div className="flex-[2] flex items-center gap-4">
                <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${emp.avatar}`} alt={emp.name} className="w-12 h-12 rounded-full border-2 border-slate-100 bg-slate-50" />
                <div>
              ...
              <div className="flex-1">
                <p className="text-base font-bold text-slate-800">{emp.salary}</p>
        ...
        <div className="px-8 py-5 border-t border-slate-100 flex items-center justify-between bg-white">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">SHOWING 1-4 OF 42 EMPLOYEES</p>
          <div className="flex items-center gap-2">
            ...
            <button className="w-8 h-8 flex items-center justify-center rounded-full text-slate-600 font-bold text-xs hover:bg-slate-50 transition-colors">2</button>
            <button className="w-8 h-8 flex items-center justify-center rounded-full text-slate-600 font-bold text-xs hover:bg-slate-50 transition-colors">3</button>
            <span className="text-slate-400 text-xs px-1">...</span>
            <button className="w-8 h-8 flex items-center justify-center rounded-full text-slate-600 font-bold text-xs hover:bg-slate-50 transition-colors">11</button>
